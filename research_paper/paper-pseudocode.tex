...
5 & \qquad $bbox \in \mathbb{R}^4$ \quad $(x_{\min}, y_{\min}, x_{\max}, y_{\max})$ in px\\
...
11 & \textbf{record} $EgoState$: $v_e \in \mathbb{R}_{\ge 0}$;\; $d_s \in \mathbb{R}_{> 0}$ \quad (speed; dist. to line)\\
12 & \textbf{record} $LeadVehicle$: $d_l, v_l \in \mathbb{R}_{\ge 0}$;\; $in\_box \in \{true, false\}$ \quad ($d_l < L_i$?)
...
2 &\\
3 & $\textsc{StoppingDistance}(v_e)$:\\
4 & \qquad \textbf{return} $v_e \cdot t_r + v_e^2 / (2 a_b)$ \quad (Eq.~1)\\
5 &\\
6 & $\textsc{ClearanceTime}(d_s, v)$:\\
7 & \qquad \textbf{return} $(d_s + L_i) / (v + 0.1)$ \quad (Eq.~2)\\
8 &\\
9 & $\textsc{IntrusionTime}(v_{lat})$:\\
10 & \qquad \textbf{return} $W_l / (|v_{lat}| + 0.1)$ \quad (Eq.~4)
\end{tabular}\end{center}
...
3 &\\
4 & $\textsc{RuleDilemma}(v_e, d_s, t_y)$:\\
5 & \qquad $d_{req} \gets \textsc{StoppingDistance}(v_e)$;\; $t_c \gets \textsc{ClearanceTime}(d_s, v_e)$\\
6 & \qquad \textbf{if} $d_s \le d_{req}$ \textbf{and} $t_c \ge t_y - \epsilon$ \textbf{then return} $Critical$ \textbf{else return} $\bot$\\
7 &\\
8 & $\textsc{RuleLead}(v_e, d_l, v_l, in\_box, t_y)$:\\
9 & \qquad $d_{req} \gets \textsc{StoppingDistance}(v_e)$\\
10 & \qquad \textbf{if} $v_l < 1.0$ \textbf{and} $d_l < d_{req}$ \textbf{and} $in\_box$ \textbf{then return} $Critical$ \quad (stopped in box)\\
11 & \qquad \textbf{if} $t_c^{eff} \ge t_y - \epsilon$ \textbf{and} $d_l < d_{req}$ \textbf{then return} $Warning$ \quad (clearance failure)\\
12 & \qquad \textbf{return} $\bot$\\
13 &\\
14 & $\textsc{RuleCutin}(D, v_e, t_y)$:\\
15 & \qquad $d_{req} \gets \textsc{StoppingDistance}(v_e)$\\
16 & \qquad \textbf{for} $o \in D$ \textbf{with} $o.cls \in \{2,3,5,7\}$:\\
17 & \qquad\quad \textbf{if} $o.signal$ \textbf{and} $o.lane \neq same$ \textbf{and} $o.age \ge k_{\min}$ \textbf{and} $|o.v_{lat}| \le 4.0$:\\
18 & \qquad\quad\quad \textbf{if} $\textsc{IntrusionTime}(o.v_{lat}) < t_y$ \textbf{and} $o.d < d_{req}$ \textbf{then return} $Warning$\\
19 & \qquad \textbf{return} $\bot$\\
20 &\\
21 & $\textsc{RuleYellow}(\ell, t_y)$:\\
22 & \qquad \textbf{if} $\ell = Yellow$ \textbf{and} $t_y < 2.5$ \textbf{then return} $Caution$ \textbf{else return} $\bot$\\
23 &\\
24 & $\textsc{RuleStale}(\ell, v_e, d_s)$:\\
25 & \qquad \textbf{if} $\ell = Green$ \textbf{and} $d_s > \textsc{StoppingDistance}(v_e)$ \textbf{then return} $Caution$ \textbf{else return} $\bot$
\end{tabular}\end{center}
...
\begin{center}\scriptsize
\begin{tabular}{@{}r@{\hspace{0.7em}}p{3.0in}@{}}
1 & $\textsc{EvaluateSafety}(v_e, d_s, D, \ell, t_y)$:\\
...
5 & \qquad\qquad $\textsc{RuleYellow}(\ell, t_y),\ \textsc{RuleStale}(\ell, v_e, d_s)\,]$\\
6 & \qquad \textbf{for} $r \in R$:\\
7 & \qquad\quad $w \gets r()$\\
8 & \qquad\quad \textbf{if} $w \neq \bot$ \textbf{then return} $w$ \quad (first match $=$ most severe)\\
9 & \qquad \textbf{return} $Safe$
\end{tabular}\end{center}
...
\begin{tabular}{@{}r@{\hspace{0.7em}}p{3.0in}@{}}
1 & $\textsc{run}(source)$:\\
2 & \qquad \textbf{for} frame $f$ \textbf{from} $CameraCapture(source)$:\\
...
7 & $\textsc{collect}(source, rate)$: save frames at $rate$ Hz for training\\
8 & $\textsc{train}$: build dataset $\to$ GPU train $\to$ export ONNX $\to$ validate
\end{tabular}\end{center}
