...
% ------------------------------------------------------------------
\title{Deterministic Intersection Blockage Prediction:\\A Kinematic Framework with Mathematical Proofs and a Modular Implementation --- Pseudocode Edition}
...
expressed as a theorem with a constructive proof; the complete mathematical
...
is evaluated by exhaustive enumeration over $15{,}840$ discretised states and
by a Monte Carlo simulation of $10{,}000$ random intersection approaches,
both in exact agreement with the theorem conditions.
\end{abstract}

\begin{IEEEkeywords}
intersection blockage, dilemma zone, kinematic safety, driver assistance,
deterministic decision making, Rust, mathematical proofs
\end{IEEEkeywords}
...
\item A complete, self-contained mathematical appendix containing every proof
...
\item A synthetic evaluation suite with exhaustive enumeration over
      $15{,}840$ discretised states and a Monte Carlo simulation of $10{,}000$ random
...
implementation.
Section~\ref{sec:verif} presents evaluation scenarios.
...
models are accurate but site-specific and carry no guarantee. This
...
large annotated corpora, are difficult to audit, and do not provide
...
(above which the detection is treated as an artifact). The mathematical guarantee
...
layer is deliberately treated as an oracle: the mathematical guarantees are
...
Because the decision engine is deterministic, its behaviour can be evaluated
...
\emph{On terminology.} The guarantees of this paper are conventional
mathematical proofs (Appendix~\ref{app:proofs}), written and checked by hand
rather than by a proof assistant, and the evaluation below proceeds by
exhaustive enumeration over a discretised grid of the bounded input space,
together with Monte Carlo sampling. We therefore use ``evaluation'' rather
than ``formal verification'': no proof assistant or model checker is used,
and none is claimed.

\begin{table}[t]
\caption{Canonical evaluation scenarios (constants of
...
space, it is exhaustively evaluated: the companion test suite
...
The evaluation suite (Table~\ref{tab:scenarios}) is deterministic and
...
outputs, which permits exhaustive evaluation and reproducible test
...
\item \textbf{Perception uncertainty.} Neural network outputs are point
  ...
  mathematical results therefore hold for the given inputs, not for the unobserved
  ground truth.
...
\item \textbf{Occlusion.} A large vehicle can hide the stop line or the lead
  vehicle entirely; such detection gaps lie outside the model's scope.
...
\paragraph{Scope of the mathematical results.} The proofs are conventional
mathematical proofs, written and checked by hand rather than by a proof
assistant, and establish properties of
...
with the guardian-angel framing: the system is a mathematically derived safety
...
$10{,}000$ random scenes used for Monte Carlo evaluation, comparing
...
driver retains full control, and the mathematical guarantees concern the warning
...
This appendix develops the mathematical results of Section~\ref{sec:kinematics}
...
The proofs below are conventional mathematical proofs, written and checked
by hand rather than by a proof assistant. Every result is a conditional
statement: given exact inputs that
...
mathematical guarantee therefore covers the decision logic itself, not the
