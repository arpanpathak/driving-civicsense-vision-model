...
\State \quad $\ell \in \{Red, Yellow, Green, Unknown\}$ \Comment{Traffic-light phase}
\State \quad $\lambda \in \{same, left, right, unknown\}$ \Comment{Lane position}
\State \quad $w \in \{Safe, Caution, Warning, Critical\}$
\State \qquad \Comment{Severity order: $Safe < Caution < Warning < Critical$}
\State
\State \textbf{Record} Detection: \Comment{Tracked object from perception pipeline}
\State \quad $\textit{bbox} \in \mathbf{R}^4$ \Comment{Bounding box $(x_{\min}, y_{\min}, x_{\max}, y_{\max})$ in px}
\State \quad $\textit{class} \in \{2,3,5,7\}$ \Comment{COCO: car, motorcycle, bus, truck}
\State \quad $v, v_{\text{lat}} \in \mathbf{R}_{\ge 0}$ \Comment{Longitudinal / lateral speed (m/s)}
\State \quad $d \in \mathbf{R}_{>0}$ \Comment{Distance from ego front bumper (m)}
\State \quad $\textit{lane} \in \lambda$ \Comment{Assigned lane}
\State \quad $\textit{signal} \in \{true, false\}$ \Comment{Turn-signal detected?}
\State \quad $\textit{age} \in \mathbf{N}$ \Comment{Consecutive frames; gates cut-in rule ($\ge k_{\min}$)}
\State
\State \textbf{Record} EgoState: \Comment{Instantaneous ego snapshot}
\State \quad $v_e \in \mathbf{R}_{\ge 0}$ \Comment{Speed (m/s), from OBD-II / GNSS / vision}
\State \quad $d_s \in \mathbf{R}_{>0}$ \Comment{Distance to stop line (m)}
\State
\State \textbf{Record} LeadVehicle: \Comment{Closest same-lane leader}
\State \quad $d_l, v_l \in \mathbf{R}_{\ge 0}$ \Comment{Distance (m), speed (m/s)}
\State \quad $\textit{in\_box} \in \{true, false\}$ \Comment{$d_l < L_i$?}
\end{algorithmic}
\end{algorithm}
...
\State $\mathbf{R}$eturn $v_e \cdot t_r + v_e^2 / (2 \cdot a_b)$ \Comment{Eq.~(1)}
\EndFunction
\State
\Function{ClearanceTime}{$d_s, v$}
\State $\mathbf{R}$eturn $(d_s + L_i) / (v + \delta)$ \qquad $(\delta = 0.1)$ \Comment{Eq.~(2)}
\EndFunction
\State
\Function{IntrusionTime}{$v_{\text{lat}}$}
\State $\mathbf{R}$eturn $W_l / (|v_{\text{lat}}| + \delta)$ \Comment{Eq.~(4)}
\EndFunction
\State
\Function{LipschitzBound}{$\Delta v$}
\State $\mathbf{R}$eturn $L \cdot \Delta v$ \Comment{Max shift of stopping boundary under sensor noise}
\EndFunction
\end{algorithmic}
\end{algorithm}

\begin{algorithm}[t]
\caption{Decision rules (\texttt{src/rules.rs}). Each returns a warning level or nil.}
\begin{algorithmic}[1]
\Function{RuleRed}{$\ell$}
\If{$\ell = Red$} $\mathbf{R}$eturn $Critical$ \EndIf
\State $\mathbf{R}$eturn $nil$
\EndFunction
\State
\Function{RuleDilemma}{$v_e, d_s, t_y$}
\State $d_{\text{req}} \gets StoppingDistance(v_e)$
\State $t_c \gets ClearanceTime(d_s, v_e)$
\State $\textit{no\_stop} \gets (d_s \le d_{\text{req}})$
\State $\textit{no\_clear} \gets (t_c \ge t_y - \epsilon)$
\If{$\textit{no\_stop} \land \textit{no\_clear}$} $\mathbf{R}$eturn $Critical$ \EndIf
\State $\mathbf{R}$eturn $nil$
\EndFunction
\State
...
\State $\mathbf{R}$eturn $Critical$ \Comment{3a: leader stopped inside box}
\EndIf
\State $t_{\text{eff}} \gets ClearanceTime(d_l, v_l)$
\If{$t_{\text{eff}} \ge t_y - \epsilon \land d_l < d_{\text{req}}$}
\State $\mathbf{R}$eturn $Warning$ \Comment{3b: clearance failure behind leader}
\EndIf
\State $\mathbf{R}$eturn $nil$
\EndFunction
\State
...
\State $t_i \gets IntrusionTime(o.v_{\text{lat}})$
\If{$t_i < t_y \land o.d < d_{\text{req}}$} $\mathbf{R}$eturn $Warning$ \EndIf
\EndIf
\EndIf
\EndFor
\State $\mathbf{R}$eturn $nil$
...
\If{$\ell = Yellow \land t_y < 2.5$} $\mathbf{R}$eturn $Caution$ \EndIf
\State $\mathbf{R}$eturn $nil$
\EndFunction
\State
\Function{RuleStale}{$\ell, v_e, d_s$}
\If{$\ell = Green \land d_s > StoppingDistance(v_e)$}
\State $\mathbf{R}$eturn $Caution$ \Comment{Worst-case: green may end at any frame}
\EndIf
\State $\mathbf{R}$eturn $nil$
\EndFunction
\end{algorithmic}
...
\State $l \gets$ first $o \in D$ with $o.\textit{lane} = same$, $o.\textit{class} \in \{2,3,5,7\}$
\If{$l \ne nil$}
\State $d_l \gets l.d,\; v_l \gets l.v,\; \textit{in\_box} \gets (l.d < L_i)$
\EndIf
\State
\State $R \gets [\; RuleRed(\ell),$
\State \qquad\; $RuleDilemma(v_e, d_s, t_y),$
\State \qquad\; $RuleLead(v_e, d_l, v_l, \textit{in\_box}, t_y),$
\State \qquad\; $RuleCutin(D, v_e, t_y),$
\State \qquad\; $RuleYellow(\ell, t_y),$
\State \qquad\; $RuleStale(\ell, v_e, d_s) \;]$
\ForAll{$r \in R$} \Comment{Evaluated in severity order; first match wins}
\State $w \gets r()$
\If{$w \ne nil$} $\mathbf{R}$eturn $w$ \EndIf
\EndFor
\State $\mathbf{R}$eturn $Safe$
\EndFunction
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}[1]
\State \textbf{Subcommand} $run(\textit{source})$:
\State \quad $F \gets CameraCapture(\textit{source})$ \Comment{CSI camera or video file}
\State \quad \ForAll{$f \in F$}
\State \qquad $D \gets YOLODetect(f)$ \Comment{ONNX Runtime, INT8 quantised}
\State \qquad $T \gets DeepSORT(D)$ \Comment{Kalman filter + Hungarian association}
\State \qquad $w \gets EvaluateSafety(v_e, d_s, T, \ell, t_y)$
\State \qquad \If{$w \ne Safe$} $DispatchAlert(w)$ \EndIf
\State \quad \EndFor
\State
\State \textbf{Subcommand} $collect(\textit{source}, \textit{rate})$:
\State \quad Save frames at \textit{rate}~Hz as JPEGs for YOLO training
\State
\State \textbf{Subcommand} $train$:
