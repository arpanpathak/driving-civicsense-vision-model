...
decision engine, which is agnostic to how the estimates are obtained. In
this paper the perception layer is deliberately treated as an oracle: the
formal guarantees are conditional on its outputs, and an end-to-end
perception error model, with covariance propagation through the kinematics,
is explicitly future work (Section~\ref{sec:discussion}).
...
/// frame; advise Caution when a comfortable stop is no longer possible.
#[must_use]
...
space, exhaustive enumeration of the discrete states and property-based
testing of the continuous inputs are straightforward to apply and are
planned alongside the field campaign of Section~\ref{sec:verif}.
