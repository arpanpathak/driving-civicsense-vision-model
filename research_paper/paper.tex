...
\begin{tikzpicture}[
  ...
  \node[pipe] (r1) at (9.5, -1.8) {rule\_dilemma\\Thm.~\ref{thm:dilemma}};
  \node[pipe] (r2) at (9.5, -2.7) {rule\_light\\red $\to$ \textsc{Critical}};
...
companion repository ships the perception infrastructure (detection,
tracking, video) and the decision engine modules listed here, with the
exhaustive test suite of Section~\ref{sec:verif}; the end-to-end rig on
vehicle hardware is the streaming ecosystem. In this paper the perception
...
pub fn evaluate_safety(
    ...
    // Build the severity-ordered pipeline. The dilemma rule precedes the
    // light rule so a Critical dilemma state is never masked by the
    // short-yellow Caution advisory.
...
monotonicity of Corollary~\ref{cor:mono}.
