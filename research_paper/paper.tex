...
\author{Arpan~Pathak,~%
\thanks{A. Pathak is the author of the CivicSense project. Correspondence: \href{mailto:author@domain.com}{author@domain.com}.}%
~CivicSense Research}
\date{}
...
vehicle is blocked---the dilemma zone in action.
...
fn main() {
    // Simulated sensor inputs (dilemma-zone scene of Figure 3a).
...
it are not. These limitations motivate two lines of future work: (i)~probabilistic
propagation of measurement uncertainty through the kinematic equations to
produce calibrated confidence levels for each warning, and (ii)~extension of
the rule set to vulnerable road users and multi-lane crossing scenes. The
deterministic core is the correct foundation for both, since probabilistic
extensions can be layered on top without changing the underlying kinematics.
...
(Appendix~\ref{app:proofs}). The Rust implementation is modular,
