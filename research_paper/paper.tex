...
\paragraph{Known vulnerabilities and operational boundaries.}
Peer review of an earlier draft surfaced six concerns that we state explicitly
rather than hide.
\textbf{(1) Signal timing.} Without V2I or a SPaT (signal phase and timing)
...
\textbf{(2) Perception uncertainty.} Neural network outputs are point
...
truth.
\textbf{(3) Lead-vehicle motion.} Theorem~\ref{thm:follow} is conditional on
a bounded leader deceleration (Remark~\ref{rem:leadbound}); the operational
envelope is defined by that bound.
\textbf{(4) HMI trust.} Frequent false positives erode driver trust and can
...
\textbf{(5) Occlusion.} A large vehicle can hide the stop line or the lead
vehicle entirely; such detection gaps lie outside the formal scope.
\textbf{(6) Road friction.} Assumption~\ref{asm:braking} fixes grip at
