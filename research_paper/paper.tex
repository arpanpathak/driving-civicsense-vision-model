...
\begin{tikzpicture}[
  ...
  % warning markers above the affected stages
  \node at (2.0, 1.05) {\warnicon~2};
  \node at (3.1, 1.05) {\warnicon~5};
  \node at (6.7, 1.05) {\warnicon~1};
  \node at (7.6, 1.05) {\warnicon~3};
  \node at (8.5, 1.05) {\warnicon~6};
  \node at (10.0, 1.05) {\warnicon~4};
  % pipeline stages
  \node[pbox] (p1) at (0.0, 0) {Video\\window};
  ...
  \draw[->, thick] (p4) -- (p5);
  % legend, two columns
  \node[align=left, anchor=west, font=\scriptsize] at (-0.3, -1.6) {
    \warnicon~1 Signal timing (V2I / SPaT)\\
    \warnicon~2 Perception uncertainty ($\pm 1.5$~m $\approx 100$~ms)\\
    \warnicon~3 Lead-vehicle motion ($\leq 0.4g$)};
  \node[align=left, anchor=west, font=\scriptsize] at (5.2, -1.6) {
    \warnicon~4 Trust erosion (false positives)\\
    \warnicon~5 Occlusion (stop line hidden)\\
    \warnicon~6 Road friction ($4$~m/s$^2$)};
\end{tikzpicture}
\caption{Known vulnerabilities mapped onto the pipeline stages they affect.
Markers: 1 signal timing, 2 perception uncertainty, 3 lead-vehicle motion,
4 HMI trust erosion, 5 occlusion, 6 road friction. Each marker denotes an
explicit operational boundary of the formal guarantees
...
rather than hide; Figure~\ref{fig:vuln} maps each onto the pipeline stage
where it enters.
\begin{enumerate}
\item[\warnicon~\textbf{1. Signal timing.}] Without V2I or a SPaT (signal
  phase and timing) feed, a vision-only system cannot know when a stale green
  turns yellow; Remark~\ref{rem:worstcase} applies, and the ``stale'' aspect
  of the warning reduces to pure geometry.
\item[\warnicon~\textbf{2. Perception uncertainty.}] Neural network outputs
  are point estimates, and a confidence score is not a formal bound. A
  bounding-box jitter of $\pm 1.5$~m at $14$~m/s shifts the arrival-time
  estimate by roughly $100$~ms, which can flip a safe decision into a blocked
  one. The formal results therefore hold for the given inputs, not for the
  unobserved ground truth.
\item[\warnicon~\textbf{3. Lead-vehicle motion.}]
  Theorem~\ref{thm:follow} is conditional on a bounded leader deceleration
  (Remark~\ref{rem:leadbound}); the operational envelope is defined by that
  bound.
\item[\warnicon~\textbf{4. HMI trust.}] Frequent false positives erode driver
  trust and can cause the system to be ignored, turning the next missed
  warning into a fatal outcome. The threshold policy is therefore
  recall-first, minimizing false negatives, and future work targets a dynamic
  threshold adapted to the driver's reaction time, with warning intensity
  modulated by confidence.
\item[\warnicon~\textbf{5. Occlusion.}] A large vehicle can hide the stop
  line or the lead vehicle entirely; such detection gaps lie outside the
  formal scope.
\item[\warnicon~\textbf{6. Road friction.}]
  Assumption~\ref{asm:braking} fixes grip at $4.0$~\si{m/s^2}; wet or icy
  surfaces reduce it, and the stopping envelope must be re-tuned for the
  local condition.
\end{enumerate}
